...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ApiErrorResponse> handleValidationException(RuntimeException ex, HttpServletRequest request) {
        return build(ex, HttpStatus.UNPROCESSABLE_ENTITY, request);
    }

    @ExceptionHandler(NotificationDeliveryException.class)
    public ResponseEntity<ApiErrorResponse> handleDeliveryException(RuntimeException ex, HttpServletRequest request) {
        log.error("Notification delivery failure: {}", ex.getMessage());
